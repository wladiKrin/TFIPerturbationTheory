...
h5open("../../data/vecs_weights_g=$(g).h5", "r") do file
    ...
    dat = sort(collect([d for d in enumerate(mapreduce(+, 1:size(weights)[1]) do i
        return occ[i].*weights[i,:]
    end) if d[2]>c]), by=x->x[2], rev=true)
    append!(is, [d[1] for d in dat])
end
@show length(is)
...
CSV.write("../../data/spec_Eff_directStates_cutoff=$(c)_L=($(L[1])_$(L[2]))_J=$(J)_g=$(g)_h=$(h).csv", dfSpec)
...
CSV.write("../../data/obs_Eff_directStates_cutoff=$(c)_L=($(L[1])_$(L[2]))_J=$(J)_g=$(g)_h=$(h).csv", df)
